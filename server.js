...
if (dev) {
  app
    ...
    .then(() => {
      ...
      server.get("/breed/:id", (req, res) =>
        app.render(req, res, "/breed", { id: req.params.id })
      );
      ...
      server.get("/api/logincustomer", async (req, res) => {
        ...
        const foundLoginData = loginInfoArray.filter(function (loginPair) {
          if (
            loginPair.email == email &&
            loginPair.orderNumber == order_number
          ) {
            return loginPair;
          }
        });
      ...
      server.post("/api/approveorder", jsonParser, async (req, res) => {
        ...
        const response = await dynamodb.approveOrder({
          order_id,
          data: { selectedBackgroundArray },
        });
        if (response.success) {
      ...
      //     const dynamodbIDArrays = new Set(
      //       ordersProofsArray.map(({ order_id }) => order_id)
      //     );
      ...
      //             successCount++;
      //             updateResponse.added.push(
      //               `${order_id} has been added to the database.`
      //             );
      //           }
      //         } else {
      //           alreadyInDBCount++;
      //           updateResponse.alreadyInDB.push(
      //             `Order ID - ${order_id} is already in the database.`
      //           );
      //         }
      //       } catch (error) {
      //         errCount++;
      //         const { order_id } = JSON.parse(error.response.config.data);
      //         updateResponse.error.push(
      //           `Order ID - ${order_id} could not be added to database. => ${error.response.data}`
      //         );
      //       }
      //     }
      ...
      server.post("/admin/api/image-upload", function (req, res) {
        proofUpload(req, res, async function (err) {
          ...
          if (response.success) {
            ...
            const updateResponse = await dynamodb.addImagesToFixture({
              order_id,
              line_items,
            });
      ...
      server.delete("/admin/api/deleteorder/:id", async (req, res) => {
        ...
        if (!(order_id && order_id.length > 0)) {
          res.status(400).json({
            success: false,
            message: `Order ID ${order_id} failed to delete`,
          });
        }
        const response = await dynamodb.deleteOrder({
          order_id,
          deleted: true,
        });
        if (response.success) {
          res.status(200).json({
            success: true,
            message: `Order ID ${order_id} successfully deleted.`,
            data: response.data,
          });
        } else {
          res.status(200).json({
      ...
      server.put("/admin/api/archiveorder/:id", async (req, res) => {
        const order_id = req.params.id;
        const isFulfilled = req.query.isfulfilled == "true";
        const response = await dynamodb.archiveOrder({
          order_id,
          data: { fulfilled: isFulfilled },
        });
      ...
      server.put("/admin/api/deleteproof/:id", jsonParser, async (req, res) => {
        ...
        const original_line_items = JSON.parse(
          JSON.stringify(req.body.line_items)
        );
        const index = req.body.index;
        line_items[index].artworkURL = "temp";
        const response = await dynamodb.deleteProof({
          order_id,
          data: { line_items },
        });
        if (response.success) {
          for (let i = 0; i < original_line_items.length; i++) {
            ...
            let imageKey = artworkURL.replace(
              "https://dwjt46l68k2gz.cloudfront.net/",
              ""
            );
            deleteImages({ key: imageKey });
          }
...
if (!dev) {
  ...
  server.get("/admin/api/pushlatest", async (req, res) => {
    ...
    const orderProofsResponse = await dynamodb.getOrders();
    const ordersProofsArray = orderProofsResponse.items;
    const relevantShopifyOrdersArray = shopifyOrdersArray.orders.map(function (
      element
    ) {
  ...
  server.put("/admin/api/archiveorder/:id", async (req, res) => {
    ...
    const fulfilled =
      shopifyResponse.data.order.fulfillment_status === "fulfilled"
        ? true
        : false;
    const response = await dynamodb.archiveOrder({
      order_id,
